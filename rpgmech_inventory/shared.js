/**
 * Inventory system tucked into a class to make this file more readable.
 * For the original script, check the inventory folder
 */
class Inventory {
  static LETTER_REGEX = /[0-9]/g;
  static DIGIT_REGEX = /\D/g;
  static PUNCTUATION_REMOVE = /[^\w\s]/gi;
  static WORN_REGEX = new RegExp(`(?<=WORN:)(.*)(?=;)`);
  static INVENTORY_REGEX = new RegExp(`(?<=INV:)(.*)(?=.)`);
   ...
   */
  static findItemInInventory(itemName) {
    console.log(`START findItemInInventory(): Looking for item "${itemName}" in player's inventory.`);
    let loweredName = itemName.toLowerCase().replace(this.PUNCTUATION_REMOVE, '');
    let itemFound = this.getInventory().find((item) => {
  ...
  static removeFromInventory(itemName, itemQuantity) {
    ...
    let index = this.getInventory().indexOf(item);
  ...
  static checkInventory() {
    ...
    if (this.getInventory().length > 0) {
      let items = this.getInventory().map((item) => item.name).join(', ');
      let itemsWorn = this.getInventory().filter((item) => item.status == 'worn').map((item) => item.name).join(', ');
   ...
   * @param {integer} itemQuantity 
   */
  static addToInventory(itemName, itemQuantity) {
    ...
    if (typeof item == 'undefined') {
      ...
      item = {
        ...
        status: 'in inventory',
        type: Inventory.getType(itemName)
      };
   ...
   * @param {string} itemName 
   */
  static equipItem(itemName) {
    console.log(`START equipItem(): equipping ${itemName}`);
    const itemNameLowerCase = itemName.toLowerCase();
    let itemToBeEquipped = this.findItemInInventory(itemNameLowerCase);
    if (typeof itemToBeEquipped != 'undefined') {
      const itemToBeEquippedIndex = this.getInventory().findIndex(x => x.name == itemToBeEquipped.name);
      ...
      if (itemToBeEquipped.type != 'weapon' && itemToBeEquipped.type != 'clothing') {
        ...
        return `\n${this.capitalize(itemNameLowerCase)} is not an equippable item.`;
      }

      let playerWorldInfo = worldEntries.find(x => x.keys.includes('you'));
      let itemsWorn = playerWorldInfo.entry.match(this.WORN_REGEX)[0];
      let oldItem = this.getInventory().find(oldItem => oldItem.status == 'worn' && oldItem.type == itemToBeEquipped.type);
      if (typeof oldItem != 'undefined') {
        const oldItemIndex = this.getInventory().findIndex(x => x.name == oldItem.name);
      ...
      itemsWorn = this.getInventory().filter((x) => x.status == 'worn')
        .map((k) => {
      ...
      playerWorldInfo.entry = playerWorldInfo.entry.replace(this.WORN_REGEX, itemsWorn);
  ...
  static debugInventory() {
    ...
    let itemsWorn = playerWorldInfo.entry.match(this.WORN_REGEX)[0];
    let itemsInInventory = playerWorldInfo.entry.match(this.INVENTORY_REGEX)[0];
    ...
    itemsInInventory = this.getInventory().map((k) => {
      console.log(`INSIDE debugInventory(): Updating player WI with inventory items`);
      return `${k.name}< quantity: ${k.quantity}>`;
    }).join('/ ');

    playerWorldInfo.entry = playerWorldInfo.entry.replace(this.WORN_REGEX, itemsWorn);
    playerWorldInfo.entry = playerWorldInfo.entry.replace(this.INVENTORY_REGEX, itemsInInventory);
  ...
  static getType(itemName) {
    const checker = (input) => {
      return this.WEAPONS.some(word => input.toLowerCase().includes(word.toLowerCase())) ? 'weapon' :
        this.CLOTHING.some(word => input.toLowerCase().includes(word.toLowerCase())) ? 'clothing' : 'misc';
    }
...
/**
 * Character Data Parser tucked into a class to make this file more readable.
 * For the original script, check the character_data_parser folder
 */
class CharacterDataParser {

  static possibleLines = [
  ...
  static generatePrompt() {
    return this.possibleLines[Math.floor(Math.random() * this.possibleLines.length)];
  }
   ...
   * @param {object} character
   */
  static parseClass(character) {
    let charClass = character.class.toLowerCase();
    switch (charClass) {
      case 'mage':
        state.charClass = RpgMechanics.classList().mage.skills;
        Inventory.addToInventory('Apprentice Mage Robes', 1);
        Inventory.equipItem('Apprentice Mage Robes');
        this.possibleLines.push(
        ...
        break;
      case 'warrior':
        state.charClass = RpgMechanics.classList().warrior.skills;
        Inventory.addToInventory('Rusty Iron Sword', 1);
        Inventory.addToInventory('Rusty Iron Armor', 1);
        Inventory.equipItem('Rusty Iron Sword');
        Inventory.equipItem('Rusty Iron Armor');
        this.possibleLines.push(
        ...
        break;
      case 'ranger':
        state.charClass = RpgMechanics.classList().ranger.skills;
        Inventory.addToInventory('Wooden Bow', 1);
        Inventory.addToInventory('Leather Tights', 1);
        Inventory.equipItem('Wooden Bow');
        Inventory.equipItem('Leather Tights');
        this.possibleLines.push(
        ...
        break;
      case 'thief':
        state.charClass = RpgMechanics.classList().thief.skills;
        Inventory.addToInventory('Rusty Iron Dagger', 1);
        Inventory.addToInventory('Leather Tights', 1);
        Inventory.equipItem('Rusty Iron Dagger');
        Inventory.equipItem('Leather Tights');
        break;
      case 'nightblade':
        state.charClass = RpgMechanics.classList().nightblade.skills;
        Inventory.addToInventory('Rusty Iron Dagger', 1);
        Inventory.addToInventory('Leather Tights', 1);
        Inventory.equipItem('Rusty Iron Dagger');
        Inventory.equipItem('Leather Tights');
        break;
      case 'bard':
        state.charClass = RpgMechanics.classList().bard.skills;
        Inventory.addToInventory('Rusty Iron Dagger', 1);
        Inventory.addToInventory('Linen Quilted Clothes', 1);
        Inventory.addToInventory('Lute', 1);
        Inventory.equipItem('Rusty Iron Dagger');
        Inventory.equipItem('Linen Quilted Clothes');
        break;
      case 'witch':
        state.charClass = RpgMechanics.classList().witch.skills;
        Inventory.addToInventory('Apprentice Mage Robes', 1);
        Inventory.equipItem('Apprentice Mage Robes');
      default:
...
/**
 * Gnurro's Placeholder Grab tucked into a class to make this file more readable.
 * Placeholder Grab has been tweaked with for readability.
 * For the original script, check his repository
 */
class BracketHandler {
   ...
   * @param {string} text
   */
  static grabAllBrackets(text) {
    for (var entry of text.match(this.BRACKETED)) {
      entry = entry.replace(this.BRACKETS, '');
   ...
   * @returns 
   */
  static grabBracket(index) {
    return (text.match(this.BRACKETED)[index].replace(this.BRACKETS, ''))
  }
}

/**
 * Gnurro's RPGMech tucked into a class to make this file more readable.
 * RPGMech has been tweaked to fit an Elder Scrolls scenario.
 * For the original script, check his repository.
 */
class RpgMechanics {
   ...
   * @returns object with skill list
   */
  static skillList() {
    return {
   ...
   * @returns object with class list
   */
  static classList() {
    return {
      mage: { skills: ['cackle', 'potBrew', 'dance', 'petHandle'] },
    ...
    // return {
    //   mage: {
    //     skills: ['melee', 'alchemy', 'conjuration', 'mysticism', 'restoration', 'destruction', 'alteration', 'illusion']
    //   },
    //   warrior: {
...
/**
 * Zaltys' name synthesizer tucked in a class for better readability
 * For the original script, check AIDCord
 */
class NameSynthesizer {
  static BADNAMES = ['Ackerson', 'Alison', 'Annah', 'Anu', 'Arat', 'Arrorn', 'Ashton', 'Azajaja', 'Big Red',
  ...
  static makeArrays() {
    ...
    list.forEach(this.addMid);
  }

  static addMid(seg) { // Generate array of possible mid-segments.
    while (seg.length > 3) {
  ...
  static synthName() {
    ...
    if (base.length < 3) { return Base; }
      ...
      while (nomen.length < name_length) {
        mid = this.shuffle(state.mid).find(x => nomen.substring(nomen.length - 2, nomen.length) == x.substring(0, 2))
        if (mid) { nomen += mid.substring(2); }
        else { name_length = 0; } // No valid segments found, just skip ahead.
      }
      var end = this.shuffle(state.end).find(x => nomen.substring(nomen.length - 1, nomen.length) == x.substring(0, 1));
  ...
  static nameReplace(text) {
    if (!state.names_setup) { this.setupNamelist(); }
    for (name of this.BADNAMES) {
      if (text.includes(name)) { text = text.replace(name, this.synthName()); }
    }
    return text;
  }
